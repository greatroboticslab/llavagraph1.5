...
Usage:
    python generate_fft_augmented.py \
...
def load_time_series_range(csv_path, start_idx, n_points):
    ...
    # Manual CSV parsing to avoid pandas dependency
    with open(csv_path, 'r') as f:
        header = f.readline()  # skip header
        lines = f.readlines()

    if start_idx + n_points > len(lines):
        # Not enough data for this offset, skip
        return None, None
...
def detect_dominant_frequency(fft_freqs, amplitude):
    ...
    # Search from index 1 to skip DC component
...
def compute_adaptive_xlim(peak_freq, nyquist):
    ...
    x_min_padded = -x_max * 0.02  # small left padding so 0Hz peak isn't cut off
    return x_min_padded, x_max_padded


def generate_fft_plot(data, time_ms, base_filename, segment_label, output_dir):
    """Compute FFT and generate a plot with adaptive x-axis range."""
    n = len(data)
    if n < 2:
        return None
    ...
    # 4. Amplitude normalization (compensate for windowing)
    amplitude = np.abs(fft_values) * (2.0 / n) * 2.0

    # 5. Normalized dB conversion (peak = 0 dB)
    amp_no_dc = amplitude[1:]
    freqs_no_dc = fft_freqs[1:]
    amplitude_db = 20 * np.log10(amp_no_dc + 1e-9)
    ...
    plt.plot(freqs_no_dc, normalized_db, linewidth=1.5, color='#0055aa')

    # Generic title — strip wave type prefix for training
    # e.g., "sine_100Hz_100Hz_10_absolute" -> "100Hz_100Hz_10_absolute"
    import re
    ...
    # Y-axis: show from min to 5dB above peak
    mask = (freqs_no_dc >= max(0, x_min)) & (freqs_no_dc <= x_max)
    if np.any(mask):
        visible_min = np.min(normalized_db[mask])
        plt.ylim(visible_min - 5, 5)
    else:
        plt.ylim(np.min(normalized_db) - 5, 5)
...
def process_directory(csv_dir, output_dir, wave_type):
    ...
    # Filter to only include frequencies <= 500Hz
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate augmented FFT plots from piezo CSVs")
    ...
    print(f"FFT Augmented Plot Generator")
