...
5-class classification: noise, sine, square, pulse, ramp.

Questions extract visual features for the decision tree:
  Q1: Tallest peak amplitude — above 250nm, 60-250nm, or below 60nm
  Q2: Peak frequency and decay shape — sharp drop vs gradual decay
  Q3: Harmonic pattern — progressively shorter peaks at regular intervals
  Q4: Two tallest peaks — amplitudes and frequencies (ratio computed by LLaMA)

Decision tree (applied by LLaMA/Qwen):
  - Peak > 250nm + 1Hz peak + smooth decay → pulse
  - Peak > 60nm:
    - Has staircase harmonic pattern:
      - 2nd/1st ratio < 15% → ramp
      - 2nd/1st ratio >= 15% → square
    - No harmonic pattern:
      - 2nd/1st ratio < 30% → sine
      - 2nd/1st ratio >= 30% → square
  - Peak <= 60nm:
    - Strong peak near 1Hz + smooth decay → pulse
    - Otherwise → noise
...
Usage:
    ...
    python gemini_train_5class.py \
        --image-dir /path/to/fft_augmented_nm \
        --output trainingData_FFT_5class.json
"""
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate 5-class FFT training data using Gemini"
    )
    parser.add_argument(
        "--image-dir", type=str, required=True,
        help="Directory with noise/sine/square/pulse/ramp subfolders of FFT PNGs"
    )
    parser.add_argument(
        "--output", type=str, default="trainingData_FFT_5class.json",
        help="Output JSON file path"
    )
    args = parser.parse_args()

    print("FFT Training Data Generator — 5 Class (Gemini 2.0 Flash)")
